...
class GetRoomView(APIView):
    ...
    def get(self, request, format=None):
        ...
        if code != None:
            ...
            return Response({'Room not found': 'Invalid group code'}, status=status.HTTP_404_NOT_FOUND)
...
class JoinRoomView(APIView):
    ...
    def post(self, request, format=None):
        ...
        if code != None:
            ...
            return Response({'Bad Request': 'Invalid code key'}, status=status.HTTP_400_BAD_REQUEST)
...
class LeaveRoomView(APIView):
    def post(self, request, format=None):
        if 'room_code' in self.request.session:
            self.request.session.pop('room_code') #deleting the room_code from user's session
